...
(function() {
  var boxes, bufferSize, calFullHeight, calHeight, calWidth, cutBox, escapeShellArg, generateConvert, i, j, len, lineWidth, loadTemplate, metrics, selectedBox, templates, tmpl, y;
  ...
  generateConvert = function() {
    ...
    return ("convert -size " + calWidth + "x" + calFullHeight + " xc:black ") + ((function() {
      ...
      for (k in metrics) {
        f = metrics[k];
        results.push("\\( " + (escapeShellArg(f.name)) + " -normalize -crop " + f.crop.cropW + "x" + f.crop.cropH + "+" + f.crop.cropX + "+" + f.crop.cropY + " -resize " + f.width + "x" + f.height + " \\) -geometry +" + f.pos.left + "+" + f.pos.top + " -composite");
      }
      return results;
    })()).join(' ') + ' out.png';
  };
